# Agent Instructions

Git and Dolt remote operations require explicit user or orchestrator authority.

## BDX activation

Ordinary requests use the repository's normal workflow and remain outside BDX. A
session enters the BDX lifecycle only when the user explicitly invokes a `bdx:*`
skill, asks for BDX/Beads tracking, or BDX auto-attach context is present. Repository
location, task size, and the presence of Beads do not activate BDX. Once activated,
the lifecycle skills may route to one another for that task.

## Shell Safety

Use explicit non-interactive flags for commands that may prompt.

## Build & Test

```bash
make test
```

## Architecture

bdx pairs Beads issues with durable Markdown under `$AGENT_HOME`:

- Beads owns agent task identity, status, priority, dependencies, comments, and
  optional agent-plane sharing.
- Markdown owns the long-form plan, context dumps, summaries, and decision record.
- Lifecycle skills (`plan`, `attach`, `check`, `dump`, `summarize`, and `close`)
  keep those layers linked; implementation and review skills consume that record.
- A live plan's `status:` is a cached Beads projection. Use
  `bdx-sync-status <id>` after external changes; never hand-edit `status:` or
  `sessions:`. Context dumps and summaries remain historical snapshots.
- Beads databases are per-project. `$AGENT_HOME/manifest.md` maps project slugs
  to repository paths so `attach`, automatic attach, and `bdc` can route across
  projects; direct cross-project commands use `bd -C <repo>`. Do not require a
  global `BEADS_DIR`.

Keep one copy of narrative: Beads fields and comments index the corresponding
Markdown artifact instead of duplicating its prose. Human trackers are separate,
explicit links or publications—not mirrored agent-task stores.

<!-- BEGIN BEADS CODEX SETUP: generated by bd setup codex -->
## Beads Issue Tracker

In a session explicitly opted into BDX/Beads, use Beads (`bd`) for durable task
tracking. Use the `beads` skill at `.agents/skills/beads/SKILL.md` (project install)
or `~/.agents/skills/beads/SKILL.md` (global install) for Beads workflow guidance,
then use the `bd` CLI for issue operations.

### Quick Reference

```bash
bd ready                # Find available work
bd show <id>            # View issue details
bd update <id> --claim  # Claim work
bd close <id>           # Complete work
bd prime                # Refresh Beads context
```

### Rules

- In an opted-in session, use `bd` for task tracking; do not create markdown TODO
  lists.
- Run `bd prime` when Beads context is missing or stale. Codex 0.129.0+ can load Beads context automatically through native hooks; use `/hooks` to inspect or toggle them.
- Keep persistent project memory in Beads via `bd remember`; do not create ad hoc memory files.

**Architecture in one line:** issues live in a local Dolt DB; sync uses `refs/dolt/data` on your git remote; `.beads/issues.jsonl` is a passive export. See https://github.com/gastownhall/beads/blob/main/docs/SYNC_CONCEPTS.md for details and anti-patterns.
<!-- END BEADS CODEX SETUP -->
