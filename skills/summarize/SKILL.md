...
Draft the summary inline on the active session model because it needs the full conversation. `--deep` escalates **only** the persona pass: that pass takes a file path and reads the written summary, so it can delegate losslessly. Never spawn a subagent to draft the summary; a subagent cannot see the session and would reconstruct it from a lossy brief.
