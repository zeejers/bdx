...
When the harness is Codex, invoke every shell tool call that runs `bd` with `sandbox_permissions: "require_escalated"`; for a direct `bd ...` command, also set `prefix_rule: ["bd"]`. Run direct `bdx-plan-orchestration ...` calls the same way with `prefix_rule: ["bdx-plan-orchestration"]`; `apply` writes a plan under `$AGENT_HOME`, which is commonly outside the repository sandbox. The installed allow rules only pre-approve escalation—they do not move a default call outside the workspace sandbox. This applies to read-only Beads commands too. If a sandboxed call reports a Dolt connection failure, retry at host level before diagnosing Dolt as down; wrappers inherit the sandbox. Claude Code and other non-Codex harnesses must ignore this section and use their normal execution path.
...
6. Write the narrative plan to `$AGENT_HOME/plan/bd-xxx-<slug>.md` (see template below) with `parent:` populated.
7. Build a temporary semantic JSON spec and run `bdx-plan-orchestration apply --plan <plan> --spec <spec>` so the deterministic generator validates and inserts the executable YAML section.
8. Cross-link: `bd update bd-xxx -d "plan: $AGENT_HOME/plan/bd-xxx-<slug>.md"` so the beads issue points at the valid plan.
9. Report the bd-id, labels, parent (if any), and path back to the user in one line.
...
## Orchestration
<Generated by `bdx-plan-orchestration`; do not hand-author this section.>
...
Every non-trivial plan gets one static `bdx.dev/orchestration/v1` manifest. The
Markdown phases describe the work for humans; the manifest binds those phases to
runners, dependencies, completion gates, and lifecycle finish steps. It contains
intent only—attempts, evidence, approvals, and live task status belong to their
runtime stores.

Create a temporary JSON spec and pass it to `bdx-plan-orchestration apply`. The
generator is the interface for executable syntax and plan insertion: supply semantic
JSON, then let it render the YAML. A representative spec is:
...
```json
{
  ...
  "phases": [
    {
      "id": "foundation",
      "title": "Establish the seam",
      "needs": [],
      "prepare": [{"skill": "bdx:care"}],
      "run": {
        "kind": "skill",
        "skill": "bdx:build-loop",
        "with": {"bead": "bd-xxx", "phase": 1}
      },
      "completion": {
        "acceptance": ["The planned contract exists"],
...
Shape the spec with these rules:

- Give every Markdown phase one stable kebab-case `id`; use `needs` as the only
  topology authority. Keep `max_parallel: 1` unless the plan explicitly establishes
  isolated write workspaces.
- Use a `skill` runner when an available skill matches the phase. Otherwise use an
  agent runner: `{"kind":"agent","agent":"default","prompt":"<bounded phase prompt>"}`.
- Use `prepare` only for skills whose output must prime the runner's context. The
  future executor must preserve that same-context ordering.
- Keep completion lanes distinct: `acceptance` is descriptive, `checks` are commands
  with optional timeouts, and `approval` is `none` or `human`. At least one lane is
  required.
- Put a human approval on unsettled policy or consequential decision phases. Put
  executable checks on implementation phases. Commands must be concrete: unresolved
  placeholders do not belong in an executable manifest.
- Keep `bdx:close` last in `finish.on_success`; `finish.on_failure` must preserve a
  resumable frontier and cannot close the task.

The generator rejects unknown fields, malformed runners, duplicate IDs, missing
dependencies, cycles, invalid completion contracts, and unsafe close ordering. On a
generator failure, report the issue ID and validation error; do not cross-link or
claim that the plan is ready.
...
- **Generated executable section.** `bdx-plan-orchestration` is the only writer for
  `## Orchestration`; edit the semantic spec and re-apply it when the manifest changes.
...
9. Write the narrative `$AGENT_HOME/plan/bd-xxx-<slug>.md` using the template above. Include `kind: agent-note`, the resolved bdx session identity in `sessions:` as a quoted string, the labels in `tags:`, the resolved parent in `parent:` (empty if none), and seed `rank:` from the chosen `-p` value using the tier table above (p0→10, p1→30, p2→50, p3→70). Do not manually add YAML under `## Orchestration`.
10. Create a unique temporary JSON spec under `/tmp`, using the Orchestration manifest contract above. Run `bdx-plan-orchestration apply --plan "$AGENT_HOME/plan/bd-xxx-<slug>.md" --spec <temp-spec>`. Remove only that exact temporary spec after the command returns. Stop and report if validation fails.
11. `bd update bd-xxx -d "plan: $AGENT_HOME/plan/bd-xxx-<slug>.md"` to cross-link only after generation succeeds.
12. Report the bd-id, labels, parent (if any), and absolute plan path back to the user in one line.
